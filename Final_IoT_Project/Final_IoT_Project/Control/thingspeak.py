import os
import sys
#from Final_IoT_Project.Final_IoT_Project.Classes.reading_DHT import Reading_DHT
...
class thingspeak(object):
    ...
    def sending_dht_data(self,json_format):
        #dht = Reading_DHT()
        #client = mqtt.Client()
    ...
    def sending_noDevices(self, json_format):
        # dht = Reading_DHT()
        # client = mqtt.Client()
        ...
        no_BT_devices = THD_json["bluetooth counter"]
        ...
        # build the payload string
        payload = "field3=" + str(no_BT_devices)

        # attempt to publish this data to the topic
        try:
            publish.single(topic, payload, hostname=self.mqttHost, transport=self.tTransport, port=self.tPort)
        except:
            print("There was an error while publishing the data to field 3.")
        return
